...
const mapOptions= {
 ...
 class DetailsSupport extends Component {
            ...
            <div  className="card">                                
                ...
                <Fragment>
                    <div className='row' style={{height:'100%'}}>                    
                        {support.data_cam?<div className='col'>     
                            <b>Ubicación: </b> {support.data_cam[0].street} {support.data_cam[0].number}, {support.data_cam[0].town}, {support.data_cam[0].township}, {support.data_cam[0].state}
                        </div> :null}                                      
                    </div>     
                    <div className='row'>
                        <div className='col'>
                        ...
                        <div className='col'>
                            <b>Problema: </b> {support.id_problem!==0?this.state.cat_problems[support.id_problem]:support.msg}
                        </div>
                    </div>
                    <div className='row'>                    
                        <div className='col'>
                        ...
                        <div className='col'>
                            <b>Contacto: </b> {support.user_email}{ticket.UserCreation?', '+ticket.UserCreation.cellphone:null}
                        </div>
                    </div> 
                </Fragment> 
                {ticket.id!==undefined?ticket.status===1?
                ...
                {ticket.id!==undefined?ticket.status>1?
                    <Fragment>
                        <div className='row' style={{height:'100%'}}>                    
                            <div className='col'>     
                                <b>Usuario {ticket.status ===2?'atendiendo ticket':'que cierro ticket'}: </b>
                                {ticket.UserUpdate.display_name} 
                            </div> 
                        </div>     
                        <div className='row'>
                            <div className='col'>
                                <b>Fecha y hora ultimo movimiento: </b> {new Date(ticket.date_update).toLocaleString()}
                            </div>
                        </div>
                        {ticket.status===3?<div className='row'>                    
                            <div className='col'>
                ...
                {ticket.id!==undefined?ticket.status===2?
  ...
  _onMapLoad = (map) =>{    
    map.setCenter(new window.google.maps.LatLng(parseFloat(this.state.support.data_cam[0].google_cordenate.split(',')[0]), parseFloat(this.state.support.data_cam[0].google_cordenate.split(',')[1])))
    new window.google.maps.Marker({
        position: { lat:parseFloat(this.state.support.data_cam[0].google_cordenate.split(',')[0]), lng:parseFloat(this.state.support.data_cam[0].google_cordenate.split(',')[1]) },
        map:  map,
        title: 'Soporte',        
    });
  }
  ...
  toProcess = async () => {
    try{
        ...
        if(res.status ===200){
            const data = res.data
            if (data.success) {
                firebaseC5.app('c5cuajimalpa').firestore().collection('support').doc(this.props.match.params.id).update({status:2})
            } else {
  ...
  toClose = async () => {
    try{
        ...
        if(res.status ===200){
            const data = res.data
            if (data.success) {
                firebaseC5.app('c5cuajimalpa').firestore().collection('support').doc(this.props.match.params.id).update({status:3})
            } else {
                alert('Error al querer atender ticket. ' + data.msg)
            }
        } else {
    ...
    componentDidMount(){
        ...
        firebaseC5.app('c5cuajimalpa').firestore().collection('support').doc(this.props.match.params.id).onSnapshot(doc=>{            
            if(doc.exists){
                let value = doc.data()
