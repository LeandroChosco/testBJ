...
export const limitZoneReducer = (state = camLimitZoneInitialState, action) => {
  switch (action.type) {
    ...
    case GET_LIMITS_CAM_RESPONSE:
      const aux_limit = (action.payload instanceof Array) && action.payload.length > 0 ? action.payload[0] : action.payload;
      return {
        ...state,
        isLoading: false,
        error: null,
