...
class App extends Component {
  ...
  async componentDidMount() {
    ...
    const limits_data = await conections.getLimitsCam();
    const { data } = limits_data;

    if (data.success && data.data && data.data.length > 0) {
      this.setState({ datosAlcaldia: data.data }, async () => {
  ...
  render() {
    return (
      ...
      <div className="fullcontainer">                
        ...
        <Route path="/cuadrantes/:id" exact render={(props) => <Cuadrantes matches={this.state.matches} chats={this.state.chats} canAccess={this.canAccess} {...props} toggleSideMenu = {this._cameraSideInfo} toggleControls={this._toggleControls}/>} />        <Route path='/personas' exact render={(props) => <Sospechosos showMatches={this.state.showMatches} chats={this.state.chats} canAccess={this.canAccess} {...props} toggleSideMenu={this._cameraSideInfo} toggleControls={this-this._toggleControls} />} />
