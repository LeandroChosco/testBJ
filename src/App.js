...
class App extends Component {
  ...
  async componentDidMount() {
    ...
    io.socket.get("/termicfiles", (data) => {
      ...
      this.setState({
        alertaCovid: data.data,
        alertaCovidTmp: covidTmp,
        alertaCovidState: true,
      });
    });
      ...
      if (data.data.camData[0].termic_type == 1) {
        notification.addNotification({
          ...
          message:
            ...
            data.data.cam_id +
            " Dirección: " +
            data.data.camData[0].street +
            " " +
            data.data.camData[0].number +
            " Col." +
            data.data.camData[0].town,
          level: "error",
          action: {
            label: "Ver detalles",
            callback: () => {
    ...
    const chatSOS = allMessage.docs.map((i) => {
      // console.log(i)
      let data = i.data();
      data.id = i.id;
      return data;
  ...
  loadData = () => {
    if (process.env.NODE_ENV === "production" || true) {
      ...
      firebase
        ...
        .onSnapshot((docs) => {
          if (
            this.state.matches.length !== docs.size &&
            this.state.showNotification &&
            !this.state.fisrtTime
          ) {
            this.showNot(
              "Match",
          ...
          if (this.state.fisrtTime) this.setState({ fisrtTime: false });
          this.setState({
      ...
      */
      ...
      firebaseC5
        ...
        .onSnapshot((docs) => {
          if (
            this.state.sos.length !== docs.size &&
            this.state.showNotification &&
            !this.state.fisrtTimeHelp
          ) {
            this.showNot(
              "SOS",
              "Nueva alerta de ayuda generada",
              "error",
              "Ver detalles",
              5,
              docs.docs[docs.docs.length - 1].id
            );
            this.setState({ reproducirSonido: true });
          }
          if (this.state.fisrtTimeHelp) this.setState({ fisrtTimeHelp: false });
          this.setState({
            sos: docs.docs.map((v) => {
              let value = v.data();
              ...
              else value.dateTime = value.date;
              value.id = v.id;
              return value;
            }),
          });
      ...
      firebaseC5cuajimalpa
        ...
        .onSnapshot((docs) => {
          if (
            this.state.support.length !== docs.size &&
            this.state.showNotification &&
            !this.state.fisrtTimeSupport
          ) {
            this.showNot(
              "Solicitud de soporte",
              "Nueva solicitud de soporte generada",
              "info",
              "Ver detalles",
              4,
              docs.docs[0].id
      ...
      firebaseC5
        ...
        .onSnapshot((docs) => {
          if (
            this.state.complaiments.length !== docs.size &&
            this.state.showNotification &&
            !this.state.fisrtTimecomplaiments
          ) {
            this.showNot(
              "Nueva denuncia",
      ...
      firebaseC5
        ...
        .onSnapshot((docs) => {
              ...
              action: {
                ...
                callback: () => {
                  ...
                  this.state.chats.forEach((chat) => {
                    if (
                      chat.user_creation == docs.docs[0].data().user_id &&
          ...
          if (this.state.fisrtTimeCall) this.setState({ fisrtTimeCall: false });
          this.setState({
      ...
      .then((response) => {
        ...
        for (let index = 0; index < data.length; index++) {
          ...
          if (element.active === 1 && element.flag_streaming === 1) {
            if (
              dns.indexOf(
                "http://" + element.UrlStreamToCameras[0].Url.dns_ip
              ) < 0
            ) {
  ...
  _makeAuth = (userInfo) => {
    sessionStorage.setItem(
      "isAuthenticated",
      JSON.stringify({ logged: true, userInfo: userInfo })
    );
  ...
  _cameraSideInfo = (cameraInfo) => {
    this.setState({
      cameraInfoSide: !this.state.cameraInfoSide,
      cameraID: cameraInfo,
    });
  };
      ...
      <Router>
        {this.state.roberyNotification.display && (
          <RoberyNotification
            ...
            userId={this.state.roberyNotification.data}
          />
        )}
        ...
        {this.state.modalCall ? (
          <ModalCall
            data={this.state.callInfo}
            modal={this.state.modalCall}
            hideModal={() => this.setState({ modalCall: false, callInfo: {} })}
          />
        ) : null}
        <div className="fullcontainer">
          ...
          <Route
            ...
            render={(props) =>
              this.state.isAuthenticated ? (
                <Redirect
                  to={{
                    pathname: this.state.userInfo.modules
          ...
          <Route
            ...
            render={(props) => (
              <Covid
                alertaCovidState={this.state.alertaCovidState}
                _alertaCovidState={this._alertaCovidState}
                _newCovidItem={this._newCovidItem}
                newCovidState={this.state.newCovidState}
                newCovidItem={this.state.newCovidItem}
                alertaCovid={this.state.alertaCovid}
        ...
        {this.state.cameraControl ? (
          <CameraControls
            camera={this.state.cameraInfo}
            toggleControls={this._toggleControls}
            active={this.state.cameraControl}
          />
        ) : null}
