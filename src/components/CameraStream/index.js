...
class CameraStream extends Component {
    ...
    return (
      <Card style={{ display: this.state.display }}>
        {this.props.horizontal ? (
          <Card.Body>
            ...
            <Card.Text>
              <Row>
                <Col lg={6}>
                  <div className="camHolder">
                    {this.props.marker.extraData.isRtmp ? (
                      ...
                      />
                    ) : this.props.marker.extraData.dataCamValue.tipo_camara ===
                      3 ? (
                      <WssPlayer data={this.props.marker.extraData} />
                ...
                <Col lg={6}>{this.state.cameraName}</Col>
              </Row>
            </Card.Text>
          ...
          <Card.Body>
            {this.props.hideTitle ? null : (
              <Card.Title>
                <div align="left">
                  <i className="fa fa-video-camera"></i> Camara{" "}
                  {this.state.num_cam}
                  {this.props.marker.extraData.dataCamValue ==
                  undefined ? null : this.props.marker.extraData.dataCamValue
                      .tipo_camara === 2 ? (
                    <i>, Tipo: PTZ</i>
                  ) : null}
                  {this.props.marker.extraData.dataCamValue ==
                        ...
                        ? this.props.moduleActions.viewHistorial
                          ? {
                              ...
                              render: () => (
                                <Tab.Pane attached={false}>
                                  <div className="row">
                                    {this.state.video_history.items
                                      ? this.state.video_history.items.map(
                                          (row, index) => (
                                            <div
                                              className="col-12"
                                              align="center"
                                            ...
                                            >
                                              ...
                                              <div className="row">
                                                {row.videos.map((value, i) => (
                                                  <MediaContainer
                                                    dns_ip={
                                                      "http://" +
                                                      this.state.video_history
                                                        .dns_ip
                                                    }
                                                    hideDelete
                                                    src={
                                                      value.RecordProccessVideo
                                                        .relative_path_file
                                                    }
                                                    value={value}
                                                    cam={this.state.data}
            ...
            <div
              className={
                this.state.showData ? "camHolder hideCamHolder" : "camHolder"
              }
              style={{ width: "100%" }}
              align="center"
            >
            ...
            {this.props.hideText ? null : (
              <div align="left">
                ...
                {this.props.hideButton ? null : (
                  <Button
                    onClick={() =>
                      this.setState({ showModalMoreInformation: true })
                    }
                    className="ml-2 mt-1"
                  >
                    Más información
                ...
                {this.state.showModalMoreInformation ? (
                  <ModalMoreInformation
                    ...
                    modal={this.state.showModalMoreInformation}
                    hide={() =>
                      this.setState({ showModalMoreInformation: false })
                    }
                    cam_id={this.state.cameraID}
                    data_cam={this.state.cameraName}
                  ></ModalMoreInformation>
                ) : null}
              </div>
            )}
            {this.props.showButtons ? (
              <Card.Footer>
                {this.props.moduleActions ? (
                  this.props.moduleActions.btnsnap ? (
                    <Button
                      basic
                      disabled={
                      ...
                      loading={this.state.loadingSnap}
                      onClick={this._snapShot}
                    >
                      <i className="fa fa-camera"></i>
                    </Button>
                  ) : null
                ...
                {this.props.moduleActions ? (
                  this.props.moduleActions.btnrecord ? (
                    <Button
                      basic
                      disabled={
                      ...
                      onClick={() => this.recordignToggle()}
                    >
                      <i
                        className={
                          this.state.isRecording
                            ? "fa fa-stop-circle recording"
                            : "fa fa-stop-circle"
                        }
                        style={{ color: "red" }}
                      ></i>
                ...
                {this.props.hideFileButton ? null : (
                  <Button
                ...
                <Button
                  ...
                  onClick={() => this.setState({ modalProblem: true })}
                >
                  {" "}
                  <i className="fa fa-warning"></i>
                </Button>
                <Button basic onClick={this._chageCamStatus}>
                  {" "}
                                        ...
                                        <div className="row">
                                          {row.videos.map((value, i) => (
                                            <MediaContainer
                                              dns_ip={
                                                "http://" +
                                                this.state.video_history.dns_ip
                                              }
                                              hideDelete
                                              src={
                                                value.RecordProccessVideo
        ...
        >
          ...
          <Modal.Body>{this.state.recordMessage}</Modal.Body>
        </Modal>

        <Modal
          size="lg"
          show={this.state.modalProblem}
          onHide={() =>
            this.setState({
              modalProblem: false,
              problemDescription: "",
        ...
        >
          ...
          <Modal.Body>
            <Form>
              ...
              {this.state.typeReport === 2 ? null : (
                <Form.Field>
                  <Label>
                    Se notificara a los numeros de emergencia registrados. Si se
                    desea agregar un telefono extra ingreselo aqui indicando la
                    lada del mismo(+525512345678).
                  </Label>
                  <Chips
              ...
              {this.state.typeReport === 2 ? null : (
                <Form.Field>
                  <Label>
                    Se notificara a los emails de emergencia registrados. Si se
                    desea agregar un email extra ingreselo aqui.
                  </Label>
                  <Chips
              ...
              <Form.Field>
                {this.state.typeReport === 2 ? (
                  <Label>
                    Se lo mas claro posible, indique si ha realizado alguna
                    accion para intentar resolver el problema.
                  </Label>
                ) : (
                  <Label>
    ...
    if (this.props.width) {
      if (this.refs.camRef.getBoundingClientRect().width === 0) {
        this.refs.camHolder.style.width =
          window.visualViewport.width * this.props.width + "px";
      } else {
        this.refs.camHolder.style.width =
          this.refs.camRef.getBoundingClientRect().width * this.props.width +
          "px";
      }
    }
    if (this.props.height) {
      if (this.refs.camRef.getBoundingClientRect().width === 0) {
        this.refs.camRef.style.height =
          (window.visualViewport.width - 50) * this.props.height + "px";
      } else {
        this.refs.camRef.style.height =
          this.refs.camRef.getBoundingClientRect().width * this.props.height +
          "px";
      }
    }
    if (this.props.showButtons) {
  ...
  _snapShot = () => {
    ...
    conections
      .snapShotV2(
        this.state.data.id ? this.state.data.id : this.props.marker.extraData.id
      )
      .then((response) => {
      ...
      .then((response) => {
        ...
        if (resHistory.success) {
          ...
          resHistory.data.items = resHistory.data.items.map((val) => {
            val.fecha = moment(val.RecordProccessVideo.datetime_start).format(
              "HH:mm"
            );
            let fecha_inicio = moment(val.RecordProccessVideo.datetime_start);
            if (items.length === 0) {
              items.push({
                dateTime: val.RecordProccessVideo.datetime_start,
                videos: [val],
              });
            } else {
              let found = false;
  ...
  _restartCamStream = async () => {
    ...
    try {
      const response = await conections.restartOneStream(
        dns,
        this.state.data.id
      );
