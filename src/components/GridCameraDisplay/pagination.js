            ...
            {currentPost.map((list, idx) => (
                <div key={idx} className="row">
                    ...
                    {list.videos.map((video, vidx) => (
                        ...
                        />
                            :
                            (<button className="btn btn-outline-primary ml-auto mr-auto mb-2" onClick={()=>download(video,dnsArray, protocolDownload)}>{`${dnsArray !== null ? video.fecha : video.fecha} - ${video.real_hour ? video.real_hour : null}`}</button>)
                            ))}
                </div>
            ))}
        </div>
