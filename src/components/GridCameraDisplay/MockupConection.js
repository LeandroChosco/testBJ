...
const MockupConection = ({ info, searchHistorialConections, renderLoading, showNotification }) => {
    ...
    return (
        <>
            ...
            {
                current_page && current_page.length > 0 ?
                    <>
                        ...
                        <hr />
                        <h3>{`Network ID: ${(current_page && current_page[0]) ? current_page[0].camera.zerotier.zerotier_network : " - "}`}</h3>
                        <ReactPaginate
                    ...
                    :
                    <div align="center">
                        <p className="big-letter">{localStorage.getItem(LANG) === "english" ? "No registers to show" : "No hay registros que mostrar"}</p>
