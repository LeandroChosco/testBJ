...
class AdvancedSearch extends Component {
		...
		return (
			<div>
				<Button variant="info" disabled={loading} onClick={() => this.setState({ showModal: true })} basic circular >
					<i className="fa fa-search-plus" /> Buscar Grabaciones
				</Button>
	...
	_onSubmit = (event) => {
		...
		let { endDate, startDate, startHour, endHour } = this.state;
		...
		if (moment(endDate).startOf('date').diff(moment(startDate).startOf('date'), 'days') > 30) return this.setState({ error: `No puede solicitar mas de 30 días.` });
